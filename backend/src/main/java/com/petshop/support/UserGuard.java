...
public final class UserGuard {
    ...
    public static AppUser requireActive(AppUserRepository users, String nickname, String action) {
        ...
        return requireAuthenticated(user, action);
    }

    public static AppUser requireAuthenticated(AppUser currentUser, String action) {
        if (currentUser == null) {
            throw new ApiException(ApiErrorCode.UNAUTHORIZED, "请先登录后再" + action);
        }
        if (Boolean.TRUE.equals(currentUser.getBlacklisted())) {
            String reason = currentUser.getBlacklistReason() == null || currentUser.getBlacklistReason().trim().isEmpty()
                    ? "账号已被限制"
                    : currentUser.getBlacklistReason().trim();
    ...
    public static AppUser requireSuperAdmin(AppUser currentUser) {
        AppUser user = requireAuthenticated(currentUser, "使用管理后台");
        if (!ROLE_SUPER_ADMIN.equals(user.getRole())) {
            throw new ApiException(ApiErrorCode.FORBIDDEN, "只有超级管理员可以访问管理后台");
        }
        return user;
